...
# Parámetros
# ========================================
file_in = "./p3/funciondeonda.txt" # Nombre del fichero de datos
file_out = "schroedinger" # Nombre del fichero de salida (sin extensión)
...
xs = frames_data[0][0]
lines = list()
for i, ys in enumerate(frames_data[0][1:]):
    if i == 0:
        # Para la función f, utiliza el color azul (por defecto)
        line, = ax.plot(xs, ys, "-", color="blue", linewidth=1)
    else:
        # Para la función g, utiliza el color rojo
        line, = ax.plot(xs, ys, "-", color="green", linewidth=1)
    lines.append(line)
